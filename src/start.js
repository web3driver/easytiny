...
const TINYPNG_URL = require('./constants').TINYPNG_URL;
...
function _checkConfig() {
  ...
  if (util.is.empty(params) || util.is.empty(params.key)) {
    log.fail(
      `tinypng api key not set, 'easytiny config --help' to see how to set api key`
    );
    opn(TINYPNG_URL);
    return false;
  }
...
function run(resize, overwrite, force) {
  ...
  confirmMode(resize, overwrite, force)
    .then(function(answers) {
      if (answers.ok) {
        if (resize) {
          setResizeParam().then(function(v) {
...
function confirmMode(resize, overwrite, force) {
  ...
  let description_overwrite = overwrite
    ? ' Original images will be overwritten directely.'
...
function doCompress(imageList, overwrite, resizeObj) {
  ...
  if (!overwrite && !!imageList.length) {
    _folderMsg = ` Check your compressed images at \'${path.join(
      cwdpath,
      _folderName
    )}\'.`;
  ...
  function _tinyImage(
    ...
    for (let i = 0; i < imageList.length; i++) {
      ...
      try {
        _promise = util.is.empty(resizeObj)
          ? tinify.fromFile(v)
          : tinify.fromFile(v).resize(resizeObj);
    ...
    if (util.is.empty(promise)) {
      ...
      Promise.all(promise)
        .then(function(v) {
          log.success(
            `Image compressing tasks all done!${_folderMsg}`,
            `You have compressed ${tinify.compressionCount} images this month!`
          );
        })
        .catch(function(err) {
          throw err;
  ...
  return inquirer([
    {
      ...
      name: 'method',
      message:
        'Choose your resize method, use `npm start --help` to see the discription of three methods.',
        ...
        return inquirer([
          {
            ...
            name: 'width',
            message:
              'Enter your target width. You must provide either a target width or a target height, but not both.',
            filter: function(value) {
              return Number(value);
            }
          },
          {
            ...
            name: 'height',
            message:
              'Enter your target height. You must provide either a target width or a target height, but not both.',
            filter: function(value) {
              return Number(value);
            }
          }
